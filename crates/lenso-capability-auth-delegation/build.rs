...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("failed to update auth-delegation snapshot: {error}"));
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("auth-delegation Descriptor or Schemas are stale: {error}")
        });
    }
    ...
    .unwrap_or_else(|error| panic!("auth-delegation generated Rust projection is stale: {error}"));
}
